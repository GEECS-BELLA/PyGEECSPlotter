...
class LongitudinalHTTAnalyzerHimg(WavefrontAnalyzer):
    ...
    def average_scan_data(self, scan_data):
        ...
        for i in range(len(file_list)):
            data = self.load_data(file_list[i])
            slopes_list.append(data)

        # Initialize average slopes with the first set of slopes
        avg_slopes = slopes_list[0]

        # Add the rest of the slopes
        for slopes in slopes_list[1:]:
            avg_slopes = wkpy.SlopesPostProcessor.apply_adder(avg_slopes, slopes)

        # Divide by the number of slopes to get the average
        avg_slopes = wkpy.SlopesPostProcessor.apply_scaler(avg_slopes, 1/len(slopes_list))
        return avg_slopes
    ...
    def analyze_data(self, data, analyzer_dict={}, bg=None):
        ...
        x0_init = analyzer_dict.get('x0', None)
        y0_init = analyzer_dict.get('y0', None)
        ...
        masked_data = self.apply_elliptical_mask(
                        data,
                        x0_init, y0_init,
                        analyzer_dict['plasma_horiztonal_pixels'],
        ...
        # ----------------------------------------------------- 
        mask_pixels = analyzer_dict.get('mask_pixels', 35)
        ...
        measured_x0, measured_y0 = self._get_centroid(-data_for_centroid)
        ...
        phase_shifts_bg = self.compute_phase_shifts(masked_data, shifts_when='background')
